    ...
    **ONLY CHANGE THE BODY OF THE act() FUNCTION**
    ...
    def act(self, obs: int) -> int:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        """
        # Q Table mapping (S,A)--> float value
        import numpy as np
        greedy_action = np.argmax([self.q_table[(obs, a)] for a in range(self.n_acts)])
        random_action = np.random.randint(0, self.n_acts)            
        ### RETURN AN ACTION HERE ###
        return greedy_action if np.random.random()>self.epsilon else random_action
...
class QLearningAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        """
        import numpy as np
        self.q_table[(obs, action)] = (1-self.alpha)*self.q_table[(obs, action)] + self.alpha*(reward+self.gamma*np.max([self.q_table[(n_obs, a)] for a in range(self.n_acts)]))
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        for i in reversed(range(len(sa_pairs))):
            ...
            if pair not in sa_pairs[0:i]:
                if pair in self.sa_counts.keys():
